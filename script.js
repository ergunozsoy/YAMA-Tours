...
function renderThemes(){document.getElementById('themeGrid').innerHTML=themes.map(t=>`<a class="pill" href="tema.html?t=${t[0]}">${t[1]}</a>`).join('')}
...
const ERAS={
 'Bugün':'Yaşayan katman: müzeler, festivaller, kültür yolu etkinlikleri. Şehir bugün de yazılmaya devam eden bir metindir.',
 '1950':'Çok partili hayat ve büyük göç: şehirlerin çeperleri genişler, gecekondu ile apartman yan yana yeni bir gündelik hayat kurar.',
 '1900':'Geç Osmanlı modernleşmesi: tramvay, fotoğrafhane, ilk oteller ve seyyah rehberleri. Pera ile suriçi arasında iki dünya.',
 '1800':'Islahat çağı: kışlalar, saat kuleleri, Batılılaşan mimari ve değişen ordu-şehir ilişkisi.',
 '1600':'Klasik çağın olgunluğu: Sinan sonrası külliyeler, menzil yolları, kervansaray ağı ve loncaların şehri.',
 '1453':'Eşik yılı: surlar, kuşatma ve imparatorluk değişimi. Konstantinopolis, Kostantiniyye olur; şehir yeniden kurulur.',
 '1200':'Selçuklu Anadolusu: kervansaraylar, medreseler, Ahi teşkilâtı. Konya ve Sivas, devrin başkent ufku.',
 'Roma':'Pax Romana: agora, tiyatro, hamam, taş yollar ve su kemerleri. Anadolu, imparatorluğun en zengin eyaletlerinden.',
 'Antik Çağ':'Polis dünyası ve öncesi: höyükler, ilk yazı, tapınaklar. Hattuşa, Troya ve İyonya kentleri bu katmanın anahtarlarıdır.'
};
(function(){
  const tl=document.querySelector('#time .timeline'); if(!tl) return;
  let box=document.createElement('div');
  box.style.cssText='margin-top:14px;padding:14px 16px;background:var(--card,#1b202a);border:1px solid var(--line,#2a303b);border-left:3px solid var(--gold,#d0a047);border-radius:12px;color:var(--muted,#bbad97);font-size:.95rem;display:none';
  tl.parentNode.appendChild(box);
  tl.querySelectorAll('span').forEach(sp=>{
    sp.style.cursor='pointer';
    sp.title='Tıkla — dönemi oku';
    sp.addEventListener('click',()=>{
      const k=sp.textContent.trim();
      tl.querySelectorAll('span').forEach(x=>x.style.borderColor='');
      sp.style.borderColor='var(--gold,#d0a047)';
      box.style.display='block';
      box.innerHTML='<strong style="color:var(--gold,#d0a047)">'+k+'</strong> — '+(ERAS[k]||'');
    });
  });
})();
...
(function(){
  ...
  const seq=[{v:true},...slides.map(el=>({el}))];
  let i=0;
  setInterval(()=>{
    i=(i+1)%seq.length;
