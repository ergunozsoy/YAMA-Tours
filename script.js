...
const translations = {
  tr: {
    ...
    why5Title: "Şeffaf kaynaklar", why5Text: "Her önemli içerikte kullanılan kaynaklar ve güven düzeni gösterilir.",
  ...
  en: {
    ...
    introText: "YAMA Culture aims to remove tourism from fast consumption and transform it into a form of discovery enriched by history, literature, archaeology, archaeology, architecture, language, and human stories.",
...
// 2. Orijinal Rotalar Listesi (Tüm görseller, duraklar ve kaynaklar geri getirilmiştir)
const routes = [
  {
    id: "istanbul",
    title: "İstanbul",
  ...
  },
  {
    ...
    region: "İç Anadolu · Ankara (Sesli Rehber)",
    image: "https://unsplash.com",
    stats: "Başlangıç Rotası · 15 kaynak · ★★★★★"
  }
];

// 3. Sesli Rehber İçerik Havuzu (Pop-up Katmanı)
const yamaGuides = {
  "anadolu-medeniyetleri": {
    ...
    content: `
      ...
      <p>Anadolu Medeniyetleri Müzesi yalnızca eserlerin sergilendiği bir müze değildir. Burası, Anadolu'nun yaklaşık on bin yıllık kültürel hafızasının bir araya geldiği eşsiz bir zaman yolculuğudur.</p>
      ...
      <h3>Müze Hakkında</h3>
      <p>Anadolu Medeniyetleri Müzesi'ne hoş geldiniz. Burada sergilenen taş baltalar, mühürler, tabletler ve heykeller insanlığın sessiz tanıklarıdır.</p>
  ...
  routeGrid.innerHTML = routes.map(route => `
    <div class="route-card" onclick="openGuide('${route.id}')">
      ...
      <div class="route-info">
        <h3>${route.title}</h3>
        <p>${route.region}</p>
        <p style="font-size: 0.8rem; color: var(--gold); margin-top: 5px;">${route.stats}</p>
      </div>
    </div>
  `).join('');
}
